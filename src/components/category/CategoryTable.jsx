...
function CategoryTable({ categories, setEditcategory ,setEditvisible,}) {

    const [hidecategory,setHidecategory] = useState(null)
 
    const categoryPublih = (status)=>{
            ...
            <table className="w-full whitespace-nowrap overflow-x-auto">
                <thead className="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-100">
                    <tr>
                        ...
                        <td className="px-4 py-2">SUB CATEGORIES</td>
                        <td className="px-4 py-2 text-center">PUBLISHED</td>
                        <td className="px-4 py-2 ">NEW SUBCTEGORY</td>
                ...
                <tbody className="bg-white divide-y divide-gray-100 text-gray-800">
                    {categories.map((category, index) => (
                        <tr key={index} className="">
                            ...
                            <td className="px-4 py-2">
                                <span className="text-sm font-semibold">{category.categoryName}</span>
                            </td>

                            <td className="px-4 py-2">
                                {
                                    category?.subCategories?.map((subCategory, subIndex) => (
                                    <span key={subIndex} className="text-xs font-light">{subCategory}{',  '}</span>
                            ...
                            <button type="button" className="btn btn-warning btn-sm">+Add new</button></span>
                            </td>
                            <td className="px-4 py-2">
